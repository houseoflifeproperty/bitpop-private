...
@interface BitTorrentSurfButtonBubbleController(Private)
...
@implementation BitTorrentSurfButtonBubbleController

+ (BitTorrentSurfButtonBubbleController*)showForParentWindow:(NSWindow*)parentWindow
            anchorPoint:(NSPoint)anchorPoint
                browser:(Browser*)browser
                profile:(Profile*)profile {
  LOG(WARNING) << "Show for parent window called";
  // Autoreleases itself on bubble close.
  return [[BitTorrentSurfButtonBubbleController alloc]
...
- (id)initWithParentWindow:(NSWindow*)parentWindow
               anchorPoint:(NSPoint)anchorPoint
                   browser:(Browser*)browser
                   profile:(Profile*)profile {
  if ((self = [super initWithWindowNibPath:@"BitTorrentSurfButtonBubble"
    ...
    [self performSelector:@selector(closeIfNotKey) withObject:nil afterDelay:5];
  }
  return self;
}
...
@end  // BitTorrentSurfButtonBubbleController
