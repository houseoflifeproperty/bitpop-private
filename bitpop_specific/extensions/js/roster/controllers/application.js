Chat.Controllers.Application = Ember.Object.extend({
  ...
  processExtensionMessage: function(request, sender, sendResponse) {
    ...
    } /*else if (request.kind == '_setFriendPresence') {
      Chat.Controllers.roster.setFriendPresence(JSON.parse(request.presence));
  ...
  _onMessage: function (message) {
    ...
    if (user && message.body) {
      ...
      for (var i = 0; i < views.length; ++i) {
        if (views[i].location.hash.length && views[i].location.hash.indexOf(user.get('jid')))
          return;
      }
